...
const ThanksData: React.FC<ThanksDataProps> = ({ steps, HeadingData }) => { // Destructure HeadingData
  ...
  return (
    <div className="container lg:py-8 py-3">
      <MainHeading Heading={HeadingData}  svg_stroke="var(--prc)" />
      <div className="grid grid-cols-1 xl:grid-cols-3 lg:grid-cols-2 gap-5 my-4">
        {steps.map((step, index) => (
          <div
            key={index}
            className="p-4  rounded-md shadow-lg text-white transition-all duration-300 border-[1px] border-[#5b5b5b99] hover:bg-gradient-to-r hover:from-[#56c1c1] hover:to-white h-full"
            style={{ cursor: "pointer" }}
          >
            <h3 className="text-xl text-tertiary  mb-2">{step.title}</h3>
            <p className="text-base mb-2">{step.content}</p>
            {step.subtitle && (
              <p className="text-xl text-tertiary  mb-2">{step.subtitle}</p>
            )}
