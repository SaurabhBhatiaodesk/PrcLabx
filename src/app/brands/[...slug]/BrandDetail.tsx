...
const BrandDetailPage: React.FC = () => {
  ...
  return (
    ...
    `}
      ...
      {/* Main Content */}
      <main className={`flex-1  bg-white transition-all duration-300 ${isSidebarOpen ? 'pt-[60px] p-4' : 'p-4'}`}>

        <button
          onClick={() => setIsSidebarOpen(prev => !prev)}
          className={`lg:mb-6 mb-2 p-2 bg-[#122d37] text-white rounded-md md:h-10  h-8 flex items-center justify-center  ${isSidebarOpen ? 'hidden' : ''}`}
        >
          <IoIosArrowForward
            className={`
      inline-block
