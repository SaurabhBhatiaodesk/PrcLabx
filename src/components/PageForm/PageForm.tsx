...
export default function CreatePage({ onSubmit }: PageFormProps) {
  ...
  const fetchPageDetails = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/admin/getPageById/${pageId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = pageId
      ? `${apiUrl}/api/admin/updatePage/${pageId}` // Update URL
      : `${apiUrl}/api/admin/createpage`; // Create URL
  ...
  return (
    ...
    >
      ...
      <FormControlLabel
        control={
          <Checkbox checked={formData.status} onChange={handleCheckboxChange} />
        }
        label="Status"
      />
      <input
